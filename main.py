...
async def get_valorant_results(limit=5):
    ...
    try:
        ...
        for match in match_elements[:limit]:
            try:
                # Extract team names
                team_elements = match.select('.match-item-vs-team-name')
                ...
                # Extract scores
                score_elements = match.select('.match-item-vs-team-score')
                ...
                event_name = event_element.select_one('div:nth-of-type(1)').text.strip() if event_element else "Unknown Event"
                event_stage = event_element.select_one('div:nth-of-type(2)').text.strip() if event_element else ""
